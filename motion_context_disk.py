...
def _guide_frame_path(data_path, index):
    return Path(data_path).with_suffix(".final.video") / f"ref2va_{int(index)}.guide.pt"


def _load_guide_frame(data_path, desc):
    path = _guide_frame_path(data_path, desc["index"])
    if not desc.get("guide_frame_cached") or not path.exists():
        return None
    return torch.load(path, map_location="cpu", weights_only=True)


def _cache_guide_frame(data_path, manifest_path, manifest, index, video):
    path = _guide_frame_path(data_path, index)
    path.parent.mkdir(parents=True, exist_ok=True)
    temp = path.with_suffix(".tmp")
    try:
        torch.save(video[-1:].to(device="cpu").clone(), temp)
        os.replace(temp, path)
    finally:
        temp.unlink(missing_ok=True)
    manifest = dict(manifest)
    segments = [dict(x) for x in manifest["segments"]]
    segments[index]["guide_frame_cached"] = True
    manifest["segments"] = segments
    _write_json_atomic(manifest_path, manifest)
    return manifest
...
def cache_full_batch_ref2va_segment(
    ...
    try:
        if not video_ready or not shift_ready:
            ...
            manifest = _cache_guide_frame(
                data_path, manifest_path, manifest, idx, rendered_video
            )
            rendered_mp4 = None
            video_profile = None
            if not video_ready:
...
def _export_live_candidate_preview(
    ...
    cached_video = isinstance(candidate.get("decoded_mp4_blob"), dict) and (
        candidate_index == 0 or "decoded_seam_shift" in candidate
    )
    current_video = None
    seam_shift = int(candidate.get("decoded_seam_shift", 0))
    if not cached_video:
        current_video, seam_shift = _render_one_final_video_segment(
            data_path, segments, candidate_index, vae, progress=progress,
        )
    ...
    try:
        if cached_video:
            _copy_blob_to_file(data_path, candidate["decoded_mp4_blob"], candidate_mp4)
        else:
            _encode_corrected_segment_video_mp4(
                ffmpeg, current_video, fps, candidate_mp4, token,
            )
        ...
        current_audio = _load_cached_decoded_audio(data_path, segments[candidate_index])
        cached_audio = current_audio is not None
        if not cached_audio:
            current_audio = _render_one_final_audio_segment(
                data_path, segments, candidate_index, audio_vae, fps,
                int(seam_shift), progress=progress,
            )
        ...
        manifest, _ = _cache_candidate_render(
            ...
            manifest,
            candidate_index,
            None if cached_video else candidate_mp4,
            None if cached_audio else current_audio,
            seam_shift=int(seam_shift),
        )
        ...
        _LOG.info(
            ...
            "shift=%d; candidate video source: %s",
            validated_count,
            candidate_index + 1,
            int(seam_shift),
            (
                "disk cache" if cached_video else (
                    "clip 1 decode" if candidate_index == 0
                    else f"pair {candidate_index}->{candidate_index + 1} decode"
                )
            ),
        )
